...
void obtainsLinesOfWords(std::string filename, std::string wordToSearch, int id_thread){
    ...
    std::cout << "Thread " << id_thread << " starts searching..." << std::endl;
...
void createThreadAndAssignLines(int n_threads, int n_lines, std::string filename, std::string wordToSearch){
    ...
    for(i=0; i<n_threads; i++){
        ...
        std::cout << "Creating thread " << id_thread << "..." << std::endl;
...
void waitCompletitionThreads(){
    ...
    std::cout << "\nWaiting for the completition of the threads..." << std::endl;
...
void welcomeMsg(){
    std::cout << "Welcome to " << COLOR_BLUE << "SS" << COLOR_RED << "O";
    std::cout << COLOR_YELLOW << "O" << COLOR_BLUE << "II" << COLOR_GREEN << "GL";
    std::cout << COLOR_RED << "E\n" << COLOR_RESET << std::endl;
}

void goodbyeMsg(){
    std::cout << "\nIt's was a pleasure for us... By " << COLOR_BLUE << "SS" << COLOR_RED << "O";
    std::cout << COLOR_YELLOW << "O" << COLOR_BLUE << "II" << COLOR_GREEN << "GL";
    std::cout << COLOR_RED << "E" << COLOR_RESET << std::endl;
}

int main(int argc, char *argv[]){

    if(argc != 4){
        std::cout << "Error. Please use: ./SSOOIIGLE <file_name> <word_to_search> <num_threads>";
        std::cout << std::endl;
        return EXIT_FAILURE;
    }
